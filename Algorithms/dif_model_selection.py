...
class NFold_to_DIFModelSelection(NFold):
    ...
    def _params_dif(self):
        return {'group' : 'group', 'focal': self.priv_group, 'model': '2PL'}

    def _initialize_report(self):
        self.folds_scores = pd.DataFrame(columns=clf_columns + get_all_measure_names() + ['DIF'])
    
    def _intialize_current_fold(self):
        self.current_fold_scores = pd.DataFrame(columns=clf_columns + get_all_measure_names() + ['DIF'])
    ...
    def _finish_reports(self):
        ...
        self.folds_scores[get_all_measure_names() + ['DIF']] = self.folds_scores[get_all_measure_names() + 
                                                                                 ['DIF']].astype('float64')
    ...
    def calculate_DIF(self, item_modeling):
        ...
        params = self._params_dif()
        methodDif = Dif(params)
        methodDif.calculate(item_modeling)
        self.current_fold_scores['DIF'] = list(methodDif.dif['DIF'])
